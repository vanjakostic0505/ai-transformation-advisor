...
import { formatValueRange, formatFte } from '../../utils/format';
...
/** The value bar is scaled against the largest opportunity so ranking reads visually. */
export function OpportunityRow({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      aria-label={`${opportunity.title} — indicative value ${formatValueRange(opportunity.value)} per year. Open assumptions and detail.`}
      className={cn(
        ...
        'sm:grid-cols-[28px_minmax(0,1fr)_200px_150px_24px] sm:gap-5 sm:px-6',
      )}
      style={{ animationDelay: `${index * 45}ms` }}
    >
      <span
        aria-hidden
        className="numeral hidden text-[13px] font-semibold text-faint sm:block"
      >
      ...
      <span className="block">
        ...
        <span
          aria-hidden
          className="mt-2 block h-1 w-full overflow-hidden rounded-full bg-line"
        >
          <span
            className={cn(
        ...
        <span className="mt-1.5 block text-[11.5px] text-faint">
          {opportunity.value.period} · {formatFte(opportunity.computed.fteEquivalent)} ·{' '}
          {CONFIDENCE_SHORT[opportunity.computed.confidence]}
        </span>
      </span>
      ...
      <ArrowRight
        aria-hidden
        className="hidden size-4 text-faint transition-all duration-200 group-hover:translate-x-0.5 group-hover:text-brand sm:block"
      />
    </button>
  );
}
